...
printf "8\n1\n" | gmx_mpi pdb2gmx -f "$MUTANT_PDB" -o brazzein_processed.gro -ignh

# ── 3. Box & solvation ────────────────────────────────────────────────────────
echo "==> Setting up simulation box..."
gmx_mpi editconf -f brazzein_processed.gro -o brazzein_newbox.gro -bt dodecahedron -d 1.5 -c

echo "==> Solvating..."
gmx_mpi solvate -cp brazzein_newbox.gro -cs spc216.gro -o brazzein_solv.gro -p topol.top

# ── 4. Ions ───────────────────────────────────────────────────────────────────
echo "==> Adding ions..."
gmx_mpi grompp -f ions.mdp -c brazzein_solv.gro -p topol.top -o ions.tpr
# Selection 13 = SOL (solvent group) for ion replacement
printf "13\n" | gmx_mpi genion -s ions.tpr -o brazzein_solv_ions.gro \
    -p topol.top -pname NA -nname CL -neutral -conc 0.15

# ── 5. Energy minimisation ────────────────────────────────────────────────────
echo "==> Running energy minimisation..."
gmx_mpi grompp -f em.mdp -c brazzein_solv_ions.gro -p topol.top -o em.tpr
...
printf "1\n1\n0\n" | gmx_mpi trjconv -s md.tpr -f md.xtc \
    -o md_cluster.xtc -pbc cluster -center -n index.ndx

printf "17\n1\n" | gmx_mpi trjconv -s md.tpr -f md_cluster.xtc \
    -fit rot+trans -o md_trajectory.xtc -n index.ndx

printf "1\n" | gmx_mpi trjconv -s md.tpr -f md_trajectory.xtc \
...
printf "51\n52\n" | gmx_mpi energy -f md.edr -o interaction_energy.xvg -xvg none
